...
export class Dashboard {
  ...
  @ViewChild('topProductsChart') topProductsChart!: {
  ...
  loadAllData() {
    forkJoin({
      ...
      next: (res: any) => {
        ...
        this.deliveryPending = this.allOrders
          .filter((order) => order.status === 'Pending')
          .length.toString();
        this.delivered = this.allOrders
          .filter((order) => order.status === 'Delivered')
          .length.toString();
        this.insideDelivery = this.allOrders
          .filter((order) => order.status === 'Delivery in process')
          .length.toString();
  ...
  getTopProducts() {
    ...
    this.allOrders.forEach((order: any) => {
      order.items.forEach((item: any) => {
        ...
        productCount.set(
          productId,
          (productCount.get(productId) ?? 0) + quantity
        );
      });
    });

    const topOrderedProduct = Array.from(productCount.entries()).map(
